...
const MyCapitalItem = ({ myCapitalItem, itemType, categroys, subcategroys, brands, models, suppliers }) => {
    ...
    useEffect(() => {
        const subCategoryNames = Array.from(new Set(myCapitalItem?.map(item => item?.subCategory?.subCategoryName)));
        setDistinctSubCategories(subCategoryNames);
        const CategoryNames = Array.from(new Set(myCapitalItem?.map(item => item?.category?.categoryName)));
        setDistinctCategories(CategoryNames);
        const zonalNames = Array.from(new Set(myCapitalItem?.map(item => item?.zonals?.zonalName)));
        setdistinctZonals(zonalNames);
    }, [myCapitalItem]);
    //console.log(distinctCategories);
    const [dataSource, setDataSource] = useState(myCapitalItem);
    ...
    const onFinish = (values) => {
        ...
        fetch(`https://computer-management-backend-iiscez3bd-nmshohel.vercel.app/api/v1/capital-item/approve-capital-item/${values?.id}`, {
            method: "POST",
            headers: {
    ...
    return (
      ...
      >
        Add a row
            ...
            <Title level={2}>My Capital Item</Title>
            <Table
