...
import { Button, Modal, Form, Input, Popconfirm, Table, Select } from 'antd';
const { Option } = Select;
...
const IssueRevinueItem = ({ capitalItem, itemType, categroys, subcategroys, brands, models, suppliers, notAssignRevenueItem, users, zonals }) => {
    ...
    useEffect(() => {
        const subCategoryNames = Array.from(new Set(notAssignRevenueItem?.map(item => item?.subCategory?.subCategoryName)));
        setDistinctSubCategories(subCategoryNames);
        const CategoryNames = Array.from(new Set(notAssignRevenueItem?.map(item => item?.category?.categoryName)));
        setDistinctCategories(CategoryNames);
        const zonalNames = Array.from(new Set(notAssignRevenueItem?.map(item => item?.zonals?.zonalName)));
        setdistinctZonals(zonalNames);
    }, [notAssignRevenueItem]);
    //console.log(distinctCategories);
    const [dataSource, setDataSource] = useState(notAssignRevenueItem);
...
export default IssueRevinueItem;